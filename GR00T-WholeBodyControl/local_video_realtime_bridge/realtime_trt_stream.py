...
class PoseWindowSmoother:
    def __init__(self, *, alpha: float) -> None:
        self.alpha = float(alpha)
        self._previous: dict[str, Any] | None = None

    def apply(self, pose_data: dict[str, Any]) -> dict[str, Any]:
        if self.alpha >= 0.999999:
            return pose_data
        if self._previous is None:
            self._previous = self._snapshot(pose_data)
            return pose_data

        import numpy as np

        alpha = float(np.clip(self.alpha, 0.0, 1.0))
        linear_keys = ("smpl_pose", "smpl_joints", "joint_pos", "joint_vel", "body_pos")
        quat_keys = ("body_quat_w", "body_quat")
        ...
        for key in linear_keys:
            ...
            if prev_arr.shape != value_arr.shape:
                continue
            pose_data[key] = ((1.0 - alpha) * prev_arr + alpha * value_arr).astype(np.float32)
        ...
        for key in quat_keys:
            ...
            prev_arr = np.asarray(prev, dtype=np.float32)
            value_arr = np.asarray(value, dtype=np.float32)
            if prev_arr.shape != value_arr.shape or prev_arr.shape[-1] != 4:
                continue
            pose_data[key] = self._smooth_quat_wxyz(prev_arr, value_arr, alpha)

        self._previous = self._snapshot(pose_data)
        return pose_data

    def _snapshot(self, pose_data: dict[str, Any]) -> dict[str, Any]:
        import numpy as np
    ...
    def _smooth_quat_wxyz(self, prev: Any, value: Any, alpha: float) -> Any:
        import numpy as np

        prev_arr = np.asarray(prev, dtype=np.float32)
        value_arr = np.asarray(value, dtype=np.float32)
        dot = np.sum(prev_arr * value_arr, axis=-1, keepdims=True)
        value_aligned = np.where(dot < 0.0, -value_arr, value_arr)
        out = (1.0 - alpha) * prev_arr + alpha * value_aligned
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Override video/camera FPS metadata before resampling to --target-fps. 0 keeps current auto-detection.",
    )
    ...
    parser.add_argument(
        "--publish-smoothing-alpha",
        type=float,
        default=1.0,
        help=(
            "Optional EMA smoothing for published pose windows. 1.0 keeps current behavior; "
            "try 0.6-0.85 to reduce real-robot chunk jitter at the cost of extra lag."
        ),
    )
    parser.add_argument(
        "--lock-initial-yaw",
...
def main() -> int:
    ...
    if args.source_fps_override < 0:
        raise ValueError("--source-fps-override must be >= 0")
    if not (0.0 < args.publish_smoothing_alpha <= 1.0):
        raise ValueError("--publish-smoothing-alpha must be in (0, 1]")
    ...
    if args.source_fps_override > 0:
        print(
            f"[runtime] source_fps override auto={source_fps:.3f} -> {args.source_fps_override:.3f}",
            flush=True,
        )
        source_fps = float(args.source_fps_override)
    ...
    pose_smoother = PoseWindowSmoother(alpha=args.publish_smoothing_alpha)
    ...
    def run_publisher() -> None:
        ...
        while not stop_event.is_set():
            ...
            if not neutral_start_gate.should_publish(pose_data):
                continue
            pose_data = pose_smoother.apply(pose_data)
